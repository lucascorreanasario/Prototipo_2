...
def update_output(data_despesa, data_receita, despesa, receita, start_date, end_date):
    ...
    # Filtre os DataFrames com base nas datas selecionadas
    df_ds = df_ds[(df_ds['Data'] >= start_date) & (df_ds['Data'] <= end_date)]
    df_rc = df_rc[(df_rc['Data'] >= start_date) & (df_rc['Data'] <= end_date)]
    ...
    # Crie um gráfico de linhas para despesas
    fig.add_trace(go.Scatter(name='Despesas', x=df_ds['Data'], y=df_ds['Acumulo'], mode='lines'))

    # Crie um gráfico de linhas para receitas
    fig.add_trace(go.Scatter(name='Receitas', x=df_rc['Data'], y=df_rc['Acumulo'], mode='lines'))
...
def graph2_show(data_receita, data_despesa, receita, despesa, start_date, end_date):
    ...
    fig = px.bar(df_final, x="Categoria", y="Valor", color='Output', barmode="group")        
    fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
